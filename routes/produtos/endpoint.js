...
router.post("/", (req, res) => {
    const { nome, emLinha } = req.body;
    const id = db.db.get('produtos').value().length + 1; // Simplesmente incrementa o ID
    const produto = Produto.criar(db.db.get("produtos"), id, nome, emLinha)
...
router.put('/:id', (req, res) => {
    const id = parseInt(req.params.id);
    const { nome, emLinha } = req.body;
    const produto = Produto.atualizar(db.db.get('produtos'), id, nome, emLinha);
    if (produto) {
        res.json(produto);
    } else {
        res.status(404).json({ message: 'Produto não encontrado' });
